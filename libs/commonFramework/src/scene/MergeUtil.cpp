#include <commonFramework/include/scene/MergeUtil.h>
...
namespace commonFramework
{
	...
	static uint32_t shiftMeshIndices(MeshData& meshData, const vector<uint32_t>& meshesToMerge)
	{
		...
		for (auto i : meshesToMerge)
		{
			...
			// for how much should we shift the indices in the mesh [m]
	...
	// All the meshesToMerge now have the same vertexOffset and individual index values are shifted by appropriate mount
	...
	void mergeScene(Scene& scene, MeshData& meshData, const std::string& materialName)
	{
		...
		// now move all the meshesToMerg to the end of the array
