...
{
public:
	virtual ~CameraPositionerInterface() {}
...
public:
	...
	// The update method should be called at every frame and take the time elapsed since the previous frame, as well as the mouse position and a mouse button pressed flag.
	// When the mouse button is pressed, we calculate a delta vector versus the previous mouse position and use it to construct a rotation quaternion.
	void update(double deltaSeconds, const glm::vec2& mousePos, bool mousePressed)
	{
		...
		// Establish camera's coordinate system to calculate the camera movement.
...
public:
	...
	void update(float deltaSeconds, const glm::vec2& mousePos, bool mousePressed)
	{
		...
		// normailzation is required to avoid "spinning" around the object 2pi times
