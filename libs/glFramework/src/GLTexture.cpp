...
namespace glFramework {
		...
		case GL_TEXTURE_2D:
		{
			...
			else
			{
				...
				if (!img)
				{
					...
					if (!img)
					{
						fprintf(stderr, "FATL ERROR: out of memory allocating image for fallback texture\n");
						exit(EXIT_FAILURE);
					}
				}
	...
	GLTexture::GLTexture(GLenum type, const std::string& filename, GLenum clamp)
		: GLTexture(type, filename.c_str(), clamp) {}

	GLTexture::GLTexture(GLenum type, const std::string& filename)
		: GLTexture(type, filename.c_str()) {}
