...
namespace glFramework
{
	...
	public:
		explicit GLMeshIndirect(const GLSceneDataType& data)
		...
		void draw(size_t numDrawCommands, const GLIndirectBuffer* buffer = nullptr) const
		{
			...
			glBindBufferBase(GL_SHADER_STORAGE_BUFFER, kIdxBind_Materials, bufferMaterials_.getHandle());
			glBindBufferBase(GL_SHADER_STORAGE_BUFFER, kIdxBind_ModelMatrices, bufferModelMatrices_.getHandle());
		...
		GLMeshIndirect(const GLMeshIndirect&) = delete;
		GLMeshIndirect(GLMeshIndirect&&) = default;
